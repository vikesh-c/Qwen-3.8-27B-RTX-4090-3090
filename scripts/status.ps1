...
$kv = [bool]($commandLine -match '(?i)--cache-type-k\s+q4_0' -and $commandLine -match '(?i)--cache-type-v\s+q4_0')
if (-not $kv) { $errors.Add('q4_kv_flags_missing') }
...
if ($reportedContext -ne [int](Get-RecipeProperty $cfg 'context' 163840)) { $errors.Add('context_mismatch') }
$gpu = $null
