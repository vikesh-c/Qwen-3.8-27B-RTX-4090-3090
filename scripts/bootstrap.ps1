...
function Get-LatestLlamaRelease {
    ...
    return Invoke-RestMethod -Uri $ApiUrl -Headers @{ Accept = 'application/vnd.github+json'; 'User-Agent' = 'qwen38-rtx4090-3090-recipe' } -TimeoutSec 30 -ErrorAction Stop
}
...
Write-Host "Bootstrap complete under $rootFull. Chat template: Froggeric v22 fixed template (embedded GGUF fallback if the external file is unavailable). Runtime defaults to the latest stable llama.cpp release; no API key or repository-local secret was created." -ForegroundColor Green
