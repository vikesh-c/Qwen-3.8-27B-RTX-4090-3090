...
$context = [int](Get-RecipeProperty $cfg 'context' 163840)
$maxContext = [int](Get-RecipeProperty $cfg 'maxContext' 163840)
...
if ($null -ne $templatePath) {
    $templateHash = [string](Get-RecipeProperty $artifacts 'chatTemplateSha256' '')
    if ([string]::IsNullOrWhiteSpace($templateHash)) { throw 'The external chat template is present but its expected SHA-256 is missing from the profile.' }
    $null = Assert-RecipeSha256 -Path $templatePath -Expected $templateHash -Label 'Froggeric chat template'
}
# Bootstrap hash-pins the external template; the embedded GGUF template remains the fallback.
...
$enableMtp = [bool](Get-RecipeProperty $mtp 'enabled' $false)
if (-not $enableMtp) { throw 'This recipe requires bundled MTP.' }
...
$arguments = @(
    ...
    '-fa', $flashAttention,
    '--jinja',
    '--cache-type-k', [string](Get-RecipeProperty $cfg 'cacheK' 'q4_0'),
    '--cache-type-v', [string](Get-RecipeProperty $cfg 'cacheV' 'q4_0'),
    '--host', $hostName,
    '--port', $port,
...
if ($null -ne $templatePath -and (Test-Path -LiteralPath $templatePath -PathType Leaf)) {
