...
function Get-RecipeKey {
    ...
    $directory = Join-Path $localAppData 'Qwen3.8-27B-24GB'
...
function Assert-RecipeGpu {
    ...
    $supported = @('NVIDIA GeForce RTX 4090', 'NVIDIA GeForce RTX 3090')
    if ($gpu.name -notin $supported -or $gpu.totalMiB -lt 24000) {
        throw "This recipe requires a 24 GiB NVIDIA GeForce RTX 4090 or RTX 3090; detected $($gpu.name) with $($gpu.totalMiB) MiB."
    }
    return $gpu
}
