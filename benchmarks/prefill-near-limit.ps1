...
if ($TargetTokens -lt 1 -or $TargetTokens -gt 162816) {
    throw 'TargetTokens must be between 1 and 162816 so the chat wrapper and one completion token remain below the 163840-token server limit.'
}
...
if ([string]::IsNullOrWhiteSpace($OutputPath)) {
    $OutputPath = Join-Path $env:TEMP ('qwen38-27b-rtx4090-3090-prefill-near-limit-{0}.json' -f (Get-Date -Format 'yyyyMMdd-HHmmss'))
}
...
if ($serverContext -ne 163840) { throw "The near-limit capacity check requires a 163840-token server; detected $serverContext." }
...
$result = [ordered]@{
    metadata = [ordered]@{
        ...
        targetDefinition = if ($TargetTokens -eq 160000) { '160000 candidate gate' } else { 'custom' }
