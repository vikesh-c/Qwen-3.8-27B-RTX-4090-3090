...
if (@($Contexts).Count -eq 0 -or @($Contexts | Where-Object { $_ -lt 1 -or $_ -gt 160000 }).Count -gt 0) { throw 'Contexts must be unique positive values no greater than 160000 so the wrapper and completion fit the 163840-token slot.' }
...
function New-PrefillPrompt([int]$TargetTokens, [int]$RunNumber) {
    ...
    # The 160000-token case leaves room below the 163840-token server slot.
...
function Invoke-Prefill([string]$Prompt, [int]$TargetTokens, [int]$RunNumber) {
    ...
    try {
        ...
        # the tested build. A run is cold when only that fixed preamble is reused and every
