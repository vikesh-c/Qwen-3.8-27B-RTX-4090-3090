...
- The launcher reuses the established `%LOCALAPPDATA%\Qwen3.8-27B-24GB\llama-server.key` credential and applies a protected user/SYSTEM/Administrators ACL when creating it. If `LLAMA_SERVER_API_KEY_FILE` is overridden, keep it outside the repository and protect it equivalently.
